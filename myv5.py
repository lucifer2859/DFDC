...
def read_img(path):
    return cv2.cvtColor(cv2.resize(cv2.imread(path), (224, 224)), cv2.COLOR_BGR2RGB)
...
import myres
checkpoint = torch.load("/home/kzd/DFDC/models/resnext50_32x4d-7cdf4587.pth")
class myresnext(myres.ResNet):
    def __init__(self, training=True):
        super(myresnext, self).__init__(block=myres.Bottleneck,
                                        layers=[3, 4, 6, 3],
                                        groups=32,
                                        width_per_group=4)

        self.load_state_dict(checkpoint)
        self.fc = nn.Sequential(nn.BatchNorm1d(2048),
                                nn.Linear(2048, 1))
...
for epoch in range(n_epochs):
    ...
    if loss < best:
      ...
      torch.save(model.state_dict(), '/home/kzd/DFDC/models/resnext50_07/' + str(cnt) + 'resnext.pth')
      cnt += 1
